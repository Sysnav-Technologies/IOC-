    ...
    <div class="span">
        ...
        <form id="form2" class="form-horizontal" action="" method="post">
            ...
            <div class="form-group">
                ...
                <div class="controls col-lg-6">
                    <input type="number" class="form-control floating-label" name="time" id="time" placeholder="Time in Minutes" value="<?php echo ($time); ?>">
                    <span class="help-inline"></span>
                </div>
            </div>
...
//validation
            if (name == '' || description == '' || time == '' || price == '') {
                ...
                $.post("lube_service/createPackage", {name: name, description: description, time: time, price: price},
                function (data) {
                    swal("Good job!", "Successfully added the New Package!", "success");
                    $('#form2')[0].reset(); //To reset form fields

                    $('#subloader').empty();
                    $('#subloader').load(buildUrl('lube_service/packages'), function () {
                        $('#subloader').hide();
