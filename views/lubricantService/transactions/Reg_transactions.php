<div class="col-lg-12">
    <form method="post" id="transactionForm" name="transactionForm">
                    ...
                    <td>
                        <select class="form-control" id="select1" name="select1">
                            <?php foreach ($customers as $customer) : ?>
                                <option value="<?php echo ($customer->id); ?>"><?php echo ($customer->id); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </td>
                    <td>
                        <select class="form-control" id="select2" onchange="getPackageAmount()" name="select2">
                            <?php foreach ($lu_packages as $package) : ?>	
                                <option value="<?php echo ($package->price); ?>"><?php echo ($package->name); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </td>
                    <td>
                        <input type="text"  class="form-control" name="Vno" id="Vno" placeholder="">
                    </td>
                    <td>
                        <input type="text"  class="form-control" name="original_amount" id="original_amount" readonly="readonly" value="">
                    </td>
                    <td>
                        <input type="text"  class="form-control" name="amount" id="amount" readonly="readonly" value="">
                    </td>
                    <td>
                        <input type="text" class="form-control" name="date" id="date"  readonly="readonly" value="<?php echo date("Y-m-d"); ?>">
                    </td>
        ...
        <div class="form-actions col-lg-12">
            <div class="col-lg-0 col-lg-1"><a href="javascript:void(0)" class="btn btn-primary btn-raised" id="reg_Lu_history"><i class="mdi-action-restore"></i> History</a></div>
...
<script type="text/javascript">
    function getPackageAmount() {
        ...
        var OriginalAmount = parseFloat(PacakgeAmount);
        document.transactionForm.original_amount.value = formatCurrency(OriginalAmount, false);
        var NewPacakgeAmount = PacakgeAmount * 0.80; //20% DISCOUNTS BEING SET.
        document.transactionForm.amount.value = formatCurrency(NewPacakgeAmount, false);
    }
...
//assigning values
            var id = $("#select1 option:selected").text();
...
//validation
            if (vehicleNo == '') {
                ...
                function (data) {
                    ...
                    $('#subloader2').empty();
                    $('#subloader2').load(buildUrl('carwash/reg_history'), function () {
                        $('#subloader2').hide();
    ...
    $('#reg_Lu_history').click(function (e2) {
        e2.preventDefault();
        var id = $(this).attr('id');
        $('#subloader2').load(buildUrl('lube_service/' + id), function () {
