...
//validation
            if (cust_id == '' || name == '' || nic == '' || address == '' || contact == '' || email == '') {
                ...
                if ((email[0])) {
                    email = $('#email').val();
                    $.post('carwash/checkCustomer', {email: email}, function (data) {
                        if (data.length < 1000) {
                            swal("Oops !", "Email already exists! Please try another Email!");
                            return false;
                        }

                        else {
                            var nic = $('#nic').val();
                            $.post('carwash/checkCustomerNic', {nic: nic}, function (data) {
                                if (data.length < 1000) {
                                    swal("Oops !", "NIC already exists! Customer Already Registered");
                                    return false;
                                }
...
// Returns successful data submission message when the entered information is stored in database.
                                    ...
                                    function (data) {
                                        ...
                                        $('#subloader').empty();
                                        $('#subloader').load(buildUrl('carwash/regular_customers'), function () {
                                            $('#subloader').hide();
    ...
    $('#nic').focusout(function () {
        if (($(this).val()) == "") {
            swal("Oops !", "Please fill NIC field");
        }
        var nic = $(this).val();
        if ((nic[0])) {
            nic = $(this).val();
